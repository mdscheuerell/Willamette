...
model {
  ...
  for(i in A:(n_yrs-age_min+n_fore)) {
    for(a in 1:A) {
      Run[i,a] <- Rec[i-a+1,a];
    }
    ## total run size
...
par_jags <- c("alpha","E_BH_a","mu_BH_a",
              ...
              "lp_age","lp_esc")
## if file exists, load it
if(file.exists(file.path(jagsdir, "fit_bh_base.rds"))) {
...
```{r fit_Ricker_cov, echo = FALSE, message = FALSE, warning = FALSE, cache = FALSE, eval=FALSE}
## params/states to return
par_jags <- c("alpha","E_Rkr_a","mu_Rkr_a",
              "beta",
              "gamma",
...
```{r fit_BH_cov, echo = FALSE, message = FALSE, warning = FALSE, cache = FALSE, eval=FALSE}
## params/states to return
par_jags <- c("alpha","E_BH_a","ln_BH_a",
              "beta",
              "gamma",
