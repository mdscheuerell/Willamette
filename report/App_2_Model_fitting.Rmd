...
4. a function to specify initial conditions. 
...
## 4. function to create JAGS inits
init_vals <- function() {
	list(alpha = 2,
	     c1 = rnorm(1,0,0.1),
	     b_inv = exp(mean(ln_dat_esc, na.rm=TRUE)),
	     p_pi = 1, p_mu = rep(1,A),
...
Now we can fit the models. Please note that the following code takes about an hour to run on a quad-core machine with 3.5 GHz Intel processors.
...
## empty table for model selection
...
for(i in 1:n_mods) {
  ## get correct covariate
  covar <- (cov_flow[,i+1] - mean(cov_flow[,i+1])) / sd(cov_flow[,i+1])
  ## fit the model
  mod_fit <- do.call(jags.parallel, mod_jags)
  saveRDS(mod_fit, file = file.path(jagsdir, paste0("jags_mod_fit_cov_", i, ".rds")))
  ## compute WAIC
  ldens <- cbind(matrix(0,mcmc_samp,(age_min)),
                 mod_fit$BUGSoutput$sims.list$lp_age)
  ldens <- ldens + mod_fit$BUGSoutput$sims.list$lp_esc
  WAIC[i] <- waic(ldens)$waic
  cc_smry_1[i,] <- mod_fit$BUGSoutput$summary["c1", c("2.5%","50%","97.5%")]
}
```

```{r JAGS_IO, message=FALSE, warning=FALSE, echo=FALSE}
if(length(grep(".rds", dir(file.path(jagsdir)), value = TRUE)) != n_mods) {
  ## list of model info for JAGS
  mod_jags <- list(data = dat_jags,
                   inits = init_vals,
                   parameters.to.save = par_jags,
...
cat(run_time_in_min, file = file.path(jagsdir, "run_time_in_min.txt"))
```
...
```{r get_WAIC, eval = FALSE}
d_WAIC <- round(WAIC-min(WAIC),1)
wt_WAIC <- exp(-0.5*d_WAIC)/sum(exp(-0.5*d_WAIC))
tbl_WAIC <- data.frame(life_stage=cov_meta$life_stage,
                       variable=sub(" of 7-day mean","",cov_meta$long_name),
                       begin=cov_meta$begin,
                       end=cov_meta$end,
                       lag=cov_meta$lag_1,
                       WAIC=round(WAIC,1),
                       d_WAIC=d_WAIC,
...
write.csv(tbl_WAIC,  row.names = FALSE,
          file = file.path(jagsdir, "tbl_WAIC.csv"))
best_idx <- which(tbl_WAIC$d_WAIC==0)
tbl_WAIC[,!(names(tbl_WAIC) %in% c("lag","WAIC"))]
```

```{r check_for_waic_tbl, echo = FALSE}
if(file.exists(file.path(jagsdir, "tbl_WAIC.csv"))) {
  tbl_WAIC <- read.csv(file.path(jagsdir, "tbl_WAIC.csv"))
  best_idx <- which(tbl_WAIC$d_WAIC==0)
  tbl_WAIC[,!(names(tbl_WAIC) %in% c("lag","WAIC"))]
} else {
  d_WAIC <- round(WAIC-min(WAIC),1)
...
```{r model_diagnostics}
best_fit <- readRDS(file.path(jagsdir, paste0("jags_mod_fit_cov_", best_idx, ".rds")))
## Rhat values for all parameters
rh <- best_fit$BUGSoutput$summary[,"Rhat"]
...
hist(rh, breaks=seq(0.99, ceiling(max(rh)/0.01)*0.01, by = 0.01), main = "",
     col=rgb(0, 0, 255, alpha = 50, maxColorValue = 255),
     border = "blue3", xlab = expression(italic(R[hat])))
...
```{r model_diagnostics_2, eval=FALSE}
## params of interest
...
## Autocorrelation
round(acf(best_fit$BUGSoutput$sims.list$ln_Rkr_a), 2)
```
